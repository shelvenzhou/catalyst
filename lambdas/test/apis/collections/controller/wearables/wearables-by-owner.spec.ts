...
describe('wearables by owner', () => {
  ...
  it(`When there is no third party registered for a collectionId, it should return an error`, async () => {
    ...
    await expectAsync(thirdPartyResolver(instance, thirdPartyFetcherInstance, collectionId)).toBeRejectedWithError(
  ...
  it(`When there a third party resolver doesn't respond, it should return an error`, async () => {
    ...
    await expectAsync(getWearablesByOwner(SOME_ADDRESS, true, contentClient, resolver)).toBeRejectedWithError(
